...
int main()
{
    ...
    draw_ground();
        ...
        gotoxy(11,5);  cout<<"YOUR SCORE IS:   "<<score;
    _sleep(5000);
...
for(int x=1;x<=2;x++)
{
    for(int j=0;j<head;j++)
    {
            ...
            if(cordx[j+1]-cordx[j]==0)
                {
                        Length_Between_2_Cordinates=cordy[j+1]-cordy[j];             flag=1;
                }
            else if(cordy[j+1]-cordy[j]==0)
                {
                        Length_Between_2_Cordinates=cordx[j+1]-cordx[j];             flag=2;
                }
                //////////////////////////////////////////////////
                if(j==0 && flag==1){check=1;    Length_Sign=Length_Between_2_Cordinates;}
                if(j==0 && flag==2){check=2;    Length_Sign=Length_Between_2_Cordinates;}
                //////////////////////////////////////////////////

                if(x==1)    {   show='O';       draw_snake_OR_ClearSnake(j,show,x);}
                if(x==2)    {   show=' ';       draw_snake_OR_ClearSnake(j,show,x);}
    }
    if(x==1){_sleep(90);}
}
...
void draw_snake_OR_ClearSnake(int j,char show,int x)
{
    int i=0;
        ...
        while(1)
        {
                if(flag==1)         {gotoxy(cordx[j],cordy[j]+i);}
                else if(flag==2)    {gotoxy(cordx[j]+i,cordy[j]);}

                if(x==1 && j==head-1 && i==Length_Between_2_Cordinates)        {cout<<'$';}
                else                                                           {cout<<show;}
...
void update_tail()
{
    if(check==1)
    {
        if(Length_Sign>0)    {cordy[0]++;}
        else                 {cordy[0]--;}
    }
    else if(check==2)
    {
        if(Length_Sign>0)   {cordx[0]++;}
        else                {cordx[0]--;}
    }
}
//******************************************************
void take_input()
{
  if(_kbhit())
    {
        InputFailed=false;
        cordx[head+1]=0;    cordy[head+1]=0;
        char c=getch();

        if     (c=='w' && m!=1 && m!=2)     { m=1; }
        else if(c=='s' && m!=2 && m!=1)     { m=2; }
        else if(c=='a' && m!=3 && m!=4)     { m=3; }
        else if(c=='d' && m!=4 && m!=3)     { m=4; }
        else
            {InputFailed=true;}

        if(InputFailed==false)
        {head++;     cordx[head]=cordx[head]+cordx[head-1];  cordy[head]=cordy[head]+cordy[head-1];}
    }

        if     (m==1)   {cordx[head]--;}
        else if(m==2)   {cordx[head]++;}
        else if(m==3)   {cordy[head]--;}
        else            {cordy[head]++;}
}
//***********************************************************************
...
void foodcheck()
{
    ...
    {
        if(cordx[1]>cordx[0]){cordx[0]--;}
        else if(cordx[1]<cordx[0]){cordx[0]++;}
        else if(cordy[1]>cordy[0]){cordy[0]--;}
        else if(cordy[1]<cordy[0]){cordy[0]++;}
